...
Contents
 ...
 - [Illustrative Example 2: Something More Complicated `LogitBoostReg`](#Illustration2)
...
## Illustrative Example 2: Something More Complicated - `LogitBoostReg`
...
For example, a `LogitBoostReg` model can be trained over the number of boosting iterations. In the [`TestingTools`](http://cran.r-project.org/web/packages/TestingTools/index.html) package, the `LogitBoostReg` function can be used to fit this model. For example:

```{r custom_lb, eval=FALSE}
mod <- LogitBoostReg(as.matrix(x), y, nIter = 51)
``` 
...
During resampling, `train` could loop over all five rows in `lbGrid` and fit five models. However, the  `predict.LogitBoostReg` function has an argument called `nIter` that can produce, in this case, predictions from `mod` for all five models.
...
Going back to the `LogitBoostReg` example, we could have:
...
For the `LogitBoostReg` model, some simple code to create these objects would be:
...
For the `LogitBoostReg` custom model object, we could use this code in the `predict` slot:
...
lbPred <- function(modelFit, newdata, preProc = NULL, submodels = NULL) {
  ...
  if(!is.null(submodels)) {
    ...
    for(j in seq(along = submodels$nIter)) {
      out[[j+1]] <- TestingTools::predict.LogitBoostReg(
        modelFit,
        newdata,
...
A few more notes:
 ...
 - In this case, the prediction function was called by namespace too (i.e. `TestingTools::predict.LogitBoostReg`). This may not seem necessary but what functions are available can vary depending on what parallel processing technology is being used. For example, the nature of forking used by `doMC` and `doParallel` tends to have easier access to functions while PSOCK methods in `doParallel` do not. It may be easier to take the safe path of using the namespace operator wherever possible to avoid errors that are difficult to track down. 

Here is a slimmed down version of the LogitBoostReg code already in the package:

```{r custom_lb_funcs, tidy = FALSE}
lbFuncs <- list(library = "TestingTools",
                loop = function(grid) {            
...
lbFuncs2$predict <- function(modelFit, newdata, preProc = NULL, submodels = NULL) 
  TestingTools::predict.LogitBoostReg(modelFit, newdata, type = "class")
lbFuncs2$loop <- NULL 
